...
def test_returns_static_when_no_stack_provided(scoreboard):
    ...
    _seed_backend_outcomes(scoreboard, "react_vite", "kimi_cli", losses=20)
...
def test_demotes_when_static_choice_keeps_losing(scoreboard):
    # Static pick for src/components/*.jsx is kimi_cli. Seed enough
    # losses to drop its rate well below the 0.4 default threshold.
    _seed_backend_outcomes(scoreboard, "react_vite", "kimi_cli", wins=1, losses=9)
    out = resolve_model_for_file(
        "src/components/X.jsx", stack="react_vite", scoreboard=scoreboard,
    )
    assert out == _BACKEND_ALTERNATIVES["kimi_cli"]
...
def test_no_demotion_below_min_samples(scoreboard):
    ...
    _seed_backend_outcomes(scoreboard, "react_vite", "kimi_cli", wins=0, losses=3)
    out = resolve_model_for_file(
        "src/components/X.jsx", stack="react_vite", scoreboard=scoreboard,
    )
    assert out == _resolve_static("src/components/X.jsx")
...
def test_no_demotion_above_threshold(scoreboard):
    ...
    _seed_backend_outcomes(scoreboard, "react_vite", "kimi_cli", wins=7, losses=3)
    out = resolve_model_for_file(
        "src/components/X.jsx", stack="react_vite", scoreboard=scoreboard,
    )
    assert out == _resolve_static("src/components/X.jsx")
...
def test_no_demotion_when_alternative_also_losing(scoreboard):
    """If we'd demote kimi_cli→copilot_cli but copilot_cli is ALSO bad,
...
def test_threshold_env_var_tightens_demotion(monkeypatch, scoreboard):
    ...
    _seed_backend_outcomes(scoreboard, "react_vite", "kimi_cli", wins=5, losses=5)
    assert resolve_model_for_file(
        "src/components/X.jsx", stack="react_vite", scoreboard=scoreboard,
    ) == _resolve_static("src/components/X.jsx")
    ...
    assert resolve_model_for_file(
        "src/components/X.jsx", stack="react_vite", scoreboard=scoreboard,
    ) == _BACKEND_ALTERNATIVES["kimi_cli"]
...
def test_min_samples_env_var_lowers_floor(monkeypatch, scoreboard):
    ...
    # leave kimi_cli alone. Drop the floor to 1 and the demotion fires.
    _seed_backend_outcomes(scoreboard, "react_vite", "kimi_cli", wins=0, losses=2)
...
def test_record_backend_partitions_by_backend(scoreboard):
    ...
    sb.record_backend("react_vite", shape, "kimi_cli", "no")
    sb.record_backend("react_vite", shape, "copilot_cli", "yes")
    sb.record_backend("react_vite", shape, "copilot_cli", "yes")

    rate_kimi = sb.backend_rate("react_vite", "kimi_cli", min_samples=1)
...
def test_record_backend_ignores_empty_inputs(scoreboard):
    sb = scoreboard
    sb.record_backend("", ["src/App.jsx"], "kimi_cli", "yes")
    sb.record_backend("react_vite", [], "kimi_cli", "yes")
    sb.record_backend("react_vite", ["src/App.jsx"], "", "yes")
    # Nothing should have landed
    assert sb.backend_rate("react_vite", "kimi_cli", min_samples=1) is None


def test_backend_rate_aggregates_across_shapes(scoreboard):
    sb = scoreboard
    sb.record_backend("react_vite", ["src/App.jsx"], "kimi_cli", "yes")
    sb.record_backend("react_vite", ["src/components/Foo.jsx"], "kimi_cli", "no")
    sb.record_backend("react_vite", ["src/styles.css"], "kimi_cli", "no")
    # 1 win / (1 win + 2 loss) = 0.333…
    rate = sb.backend_rate("react_vite", "kimi_cli", min_samples=3)
...
def test_backend_rate_returns_none_below_min_samples(scoreboard):
    sb = scoreboard
    sb.record_backend("react_vite", ["a.js"], "kimi_cli", "no")
    sb.record_backend("react_vite", ["b.js"], "kimi_cli", "no")
    assert sb.backend_rate("react_vite", "kimi_cli", min_samples=5) is None
...
def test_by_backend_roundtrips_through_persistence(tmp_path):
    ...
    assert sb2.backend_rate("react_vite", "kimi_cli", min_samples=1) == 0.5
